...
- `ISYE_MIRROR_USER`: an account username that can push to the ISYE repo. This is stored as a `variable`, which means anyone can see it.
- `ISYE_MIRROR_TOKEN`: a *Personal Access Token* (PAT) generated by the account, like a password. This is stored as a `secret`, which means no one can see it.
- `ISYE_MIRROR_REPO`: the repository to mirror to, **without** the https://. For example, `github.com/isye-web/cec-tmm-hai`.
...
You _do_ need to set the `ISYE_MIRROR_REPO` variable, which just indicates which repo this script should mirror to.
